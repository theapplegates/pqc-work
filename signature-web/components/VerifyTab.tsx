...
function parseClearSignedMessage(input: string): { message: string; signatureBlock: string } | null {
    ...
    const sigStartIdx = SIG_STARTS
        .map(header => normalized.indexOf(header))
        .filter(index => index !== -1)
        .sort((a, b) => a - b)[0] ?? -1;

    if (!normalized.includes(SIGNED_MSG_HEADER) || sigStartIdx === -1) {
        return null;
    }
...
export const VerifyTab: React.FC = () => {
    ...
    const handleVerify = async () => {
        ...
        setTimeout(async () => {
            try {
                // Sequoia verifies directly from armored cert + armored signature.
                const result = await verify(publicKeyPgp, msgToVerify, sigToVerify);
                ...
                <>
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder={"Comment: User ID:\t...\nComment: Valid from:\t...\nComment: Type:\tSLH-DSA-SHAKE-256s...\nComment: Subkey:\tML-KEM-1024+X448...\nComment: Fingerprint:\t...\n\nYour original message here..."}
                            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 transition font-mono text-sm"
                        />
                    </div>
