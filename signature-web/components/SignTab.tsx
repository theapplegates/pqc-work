...
export const SignTab: React.FC<Props> = ({ keys }) => {
  ...
  const handleSign = async () => {
    ...
    setTimeout(async () => {
        try {
            let privateKeyInput = selectedKey.privateKeyRaw;
            if (isKeyEncrypted) {
                if (!passphrase) {
                ...
                try {
                    privateKeyInput = await decryptPrivateKey(selectedKey, passphrase);
                } catch (e) {
            ...
            const result = await sign(privateKeyInput, message, {
